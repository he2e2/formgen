...
describe('generateZodSchema', () => {
  it('text form 유효성 검사를 통과한다.', () => {
    ...
    if (!res2.success) {
      const titleError = res2.error.issues.find((issue) => issue.path.includes('title'));
  ...
  it('custom form 유효성 검사를 통과한다.', () => {
    ...
    const result = s.safeParse({ nickname: 'foo' });
    expect(result.success).toBe(false);
    if (!result.success) {
      const nicknameError = result.error.issues.find((issue) => issue.path.includes('nickname'));
      expect(nicknameError?.message).toContain('5글자 이상');
    }
